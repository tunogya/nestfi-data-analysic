...
const handleExecute = async (tx, chainId) => {
  ...
  const ethprice = BigNumber.from('0x' + tx.input.slice(10, 74)).div(BigNumber.from(10).pow(12)).toNumber() / 1000000;
  const btcprice = BigNumber.from('0x' + tx.input.slice(74, 138)).div(BigNumber.from(10).pow(12)).toNumber() / 1000000;

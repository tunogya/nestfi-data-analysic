...
const handleNewBuyRequest = async (tx, chainId) => {
  ...
  const stopLossPrice = BigNumber.from('0x' + tx.input.slice(458, 522)).div(BigNumber.from(10).pow(14)).toNumber() / 10000;
  const takeProfitPrice = BigNumber.from('0x' + tx.input.slice(394, 458)).div(BigNumber.from(10).pow(14)).toNumber() / 10000;
